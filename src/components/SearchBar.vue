<template>
  <header>
    <h1>boolflix</h1>
    <input type="text" v-model="searchName" />
    <button @click="variazione">{{ tipo }}</button>
    <button @click="itemFound">Cerca</button>
  </header>
</template>

<script>
import state from "../store";
import axios from "axios";
export default {
  name: "SearchBar",
  props: {},
  data() {
    return {
      searchName: "",
      listaTrovati: [],
      apiPreMov: "https://api.themoviedb.org/3/search/movie?",
      apiPreTv: 'https://api.themoviedb.org/3/search/tv?',
      requestMov: axios.get(this.apiPreMov, {
            params: {
              api_key: "0b5ba28421cd5d17697f81dbe2dd182a",
              query: this.searchName,
            },
          }),
      requestTv: axios.get(this.apiPreTv, {
            params: {
              api_key: "0b5ba28421cd5d17697f81dbe2dd182a",
              query: this.searchName,
            },
          }),
      tipo : 'movie'
    };
  },
  methods: {
    itemFound() {
      if (this.searchName !== "") {
        /*axios.all([this.requestMov, this.requestTv]).then(axios.spread((res) =>{
          console.log(res)
        }));*/
        axios
          .get(this.apiPreMov, {
            params: {
              api_key: "0b5ba28421cd5d17697f81dbe2dd182a",
              query: this.searchName,
            },
          })
          .then((res) => {
            console.log(res);
            this.listaTrovati.push(res.data.results);
            state.ricerca(res.data.results);
          });
        axios
          .get(this.apiPreTv, {
            params: {
              api_key: "0b5ba28421cd5d17697f81dbe2dd182a",
              query: this.searchName,
            },
          })
          .then((res) => {
            console.log(res);
            this.listaTrovati.push(res.data.results);
            state.compleition(res.data.results)
            
          })
      }
      state.tipo = this.tipo
    },
    variazione() {
      if (this.tipo === "movie") {
        this.tipo = "tv";
      } else {
        this.tipo = "movie";
      }
    },
  },
};
</script>

<style scoped lang="scss">
</style>
